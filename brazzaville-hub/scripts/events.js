...
function renderEvents(events) {
  ...
  events.forEach((e) => {
    ...
    title.innerHTML = `<strong>${escapeHtml(
      e.title
    )}</strong><div class="meta">${formatDateRange(
      e.start,
      e.end
    )} • ${escapeHtml(e.location || "Online")}</div>`;

    const action = document.createElement("div");
    action.innerHTML = `<a href="${encodeURI(
      e.url || "#"
    )}" class="btn" aria-label="More about ${escapeHtml(e.title)}">Details</a>`;
